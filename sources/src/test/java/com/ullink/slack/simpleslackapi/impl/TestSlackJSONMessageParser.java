...
public class TestSlackJSONMessageParser {
	...
	private static final String TEST_REACTION_ADDED = "{\"type\":\"reaction_added\", "
			+ TEST_REACTION + ", \"user\":\"TESTUSER1\",\"item_user\":\"TESTUSER2\"}";
	private static final String TEST_REACTION_REMOVED = "{\"type\":\"reaction_removed\", "
			+ TEST_REACTION + ",\"user\":\"TESTUSER1\",\"item_user\":\"TESTUSER2\"}";
	...
	public void setup() {
		session = new AbstractSlackSessionImpl() {
			...
			@Override
			public void connect() {
				SlackUser user1 = new SlackUserImpl("TESTUSER1", "test user 1", "", "", "testSkype",
						"testPhone", "testTitle", false, false, false, false, false, false, false,
						"tz", "tzLabel", new Integer(0), SlackPersona.SlackPresence.ACTIVE);
				SlackUser user2 = new SlackUserImpl("TESTUSER2", "test user 2", "", "", "testSkype",
						"testPhone", "testTitle", false, false, false, false, false, false, false,
						"tz", "tzLabel", new Integer(0), SlackPersona.SlackPresence.ACTIVE);
				SlackUser user3 = new SlackUserImpl("TESTUSER3", "test user 3", "", "", "testSkype",
						"testPhone", "testTitle", false, false, false, false, false, false, false,
						"tz", "tzLabel", new Integer(0), SlackPersona.SlackPresence.ACTIVE);
				...
				SlackIntegration integration = new SlackIntegrationImpl("TESTINTEGRATION1",
						"integration 1", false);

				integrations.put(integration.getId(), integration);

				SlackChannel channel1 = new SlackChannelImpl("TESTCHANNEL1", "testchannel1", null,
						null, false, false, false);
				SlackChannel channel2 = new SlackChannelImpl("TESTCHANNEL2", "testchannel2", null,
						null, false, false, false);
				SlackChannel channel3 = new SlackChannelImpl("TESTCHANNEL3", "testchannel3", null,
						null, false, false, false);
				SlackChannel channel4 = new SlackChannelImpl("NEWCHANNEL", "new channel",
						"To have something new", "This channel so new it aint even old yet", false,
						false, false);
			...
			@Override
			public SlackMessageHandle<SlackMessageReply> sendFile(SlackChannel channel, byte[] data,
					String fileName) {
				return null;
			}

			@Override
			public SlackMessageHandle<SlackMessageReply> sendFileToUser(SlackUser user, byte[] data,
					String fileName) {
				return null;
			}

			@Override
			public SlackMessageHandle<SlackMessageReply> sendFileToUser(String userName,
			...
			@Override
			public SlackMessageHandle updateMessage(String timeStamp, SlackChannel channel,
					String message) {
				return null;
			}

			@Override
			public SlackMessageHandle addReactionToMessage(SlackChannel channel,
					String messageTimeStamp, String emojiCode) {
				return null;
			}

			@Override
			public SlackMessageHandle<SlackMessageReply> removeReactionFromMessage(
			...
			@Override
			public SlackMessageHandle<SlackChannelReply> setChannelTopic(SlackChannel channel,
					String topic) {
				return null;
			}
			...
			@Override
			public SlackMessageHandle<SlackChannelReply> openMultipartyDirectMessageChannel(
					SlackUser... users) {
				return null;
			}
			...
			@Override
			public SlackMessageHandle inviteUser(String email, String firstName,
					boolean setActive) {
				return null;
			}
			...
			@Override
			public SlackMessageHandle<SlackChannelReply> inviteToChannel(SlackChannel channel,
					SlackUser user) {
				return null;
			}

			@Override
			public SlackMessageHandle sendMessageToUser(SlackUser user, String message,
					SlackAttachment attachment) {
				throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
			}

			@Override
			public SlackMessageHandle sendMessageToUser(String userName, String message,
					SlackAttachment attachment) {
				throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
			}
	...
	public void testParsingMessageDeleted() throws Exception {
		...
		Assertions.assertThat(slackMessageDeleted.getMessageTimestamp())
				.isEqualTo("1358878749.000002");
	...
	public void testParsingMessageChanged() throws Exception {
		...
		Assertions.assertThat(slackMessageUpdated.getMessageTimestamp())
				.isEqualTo("1413187521.000005");
	...
	public void testChannelCreated() throws Exception {
		...
		Assertions.assertThat(slackChannelCreated.getSlackChannel().getName())
				.isEqualTo("new channel");
		Assertions.assertThat(slackChannelCreated.getSlackChannel().getId())
				.isEqualTo("NEWCHANNEL");
	...
	public void testChannelDeleted() throws Exception {
		...
		Assertions.assertThat(slackChannelDeleted.getSlackChannel().getId())
				.isEqualTo("TESTCHANNEL1");
	}
	...
	public void testChannelArchived() throws Exception {
		...
		Assertions.assertThat(slackChannelArchived.getSlackChannel().getId())
				.isEqualTo("TESTCHANNEL1");
	...
	public void testChannelUnarchived() throws Exception {
		...
		Assertions.assertThat(slackChannelUnarchived.getSlackChannel().getId())
				.isEqualTo("TESTCHANNEL1");
	...
	public void testGroupJoined() throws Exception {
		...
		Assertions.assertThat(slackGroupJoined.getSlackChannel().getName())
				.isEqualTo("new channel");
		Assertions.assertThat(slackGroupJoined.getSlackChannel().getTopic())
				.isEqualTo("To have something new");
		Assertions.assertThat(slackGroupJoined.getSlackChannel().getPurpose())
				.isEqualTo("This channel so new it aint even old yet");
	...
	public void testUserChange() {
		...
		Assertions.assertThat(session.findUserById("TESTUSER1").getUserName())
	...
	public void testAttachment() {
		...
		Assertions.assertThat(attachment.getFallback())
				.isEqualTo("Required plain-text summary of the attachment.");
		Assertions.assertThat(attachment.getColor()).isEqualTo("#36a64f");
		Assertions.assertThat(attachment.getPretext())
				.isEqualTo("Optional text that appears above the attachment block");
		...
		Assertions.assertThat(attachment.getAuthorIcon())
				.isEqualTo("http://flickr.com/icons/bobby.jpg");
		...
		Assertions.assertThat(attachment.getText())
				.isEqualTo("Optional text that appears within the attachment");
		Assertions.assertThat(attachment.getThumbUrl())
				.isEqualTo("http://example.com/path/to/thumb.png");
		Assertions.assertThat(attachment.getFooter()).isEqualTo("Slack API");
		Assertions.assertThat(attachment.getFooterIcon())
				.isEqualTo("https://platform.slack-edge.com/img/default_application_icon.png");
